...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory {
        ...
        private Map<String, Integer> countClass = new HashMap<>();
        private static AtomicInteger count = new AtomicInteger(1);
        private Map<String, AtomicInteger> countFactory = new HashMap<>();
        ...
        public Thread newThread(Runnable r) {
            ...
            synchronized (countClass) {
                if (!countClass.containsKey(thread.getThreadGroup().getName())) {
                    countClass.put(thread.getThreadGroup().getName(), count.getAndIncrement());
                }
            }
            synchronized (countFactory) {
                if (!countFactory.containsKey(thread.getThreadGroup().getName())) {
                    countFactory.put(thread.getThreadGroup().getName(), new AtomicInteger(1));
                }else {
                    AtomicInteger buffer = countFactory.get(thread.getThreadGroup().getName());
                    countFactory.put(thread.getThreadGroup().getName(), new AtomicInteger(buffer.incrementAndGet()));
                }
            }
            thread.setName(thread.getThreadGroup().getName() + "-pool-" + countClass.get(thread.getThreadGroup().getName()) + "-thread-" + countFactory.get(thread.getThreadGroup().getName()));
